...
def simulate_equity(our_hand: str, villain_ranges: List[Dict[str, List[Dict[str, float]]]], board: str, iterations: int) -> Dict[str, float]:
    ...
    cur_time = time.time()
    print(f"Games played : {games_played} in {cur_time - start} seconds.")
    ...
    for board in boards[:1]:
        ...
        for our_hand in our_hands[:]:
            ...
            equity = simulate_equity(
                our_hand, villain_ranges, board, num_simulations)
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(
            process_query, query, boards, ranges, NUM_SIMULATIONS) for query in queries[2:3]]
    ...
    print(
        f"Simulation completed. Results saved to {output_file} in {int(end-start)} seconds.")
